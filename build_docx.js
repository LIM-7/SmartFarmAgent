...
children.push(bullet("机会：2025—2026 年竞赛集中在「智能体 / IoT + 大模型」方向；司农、稷丰等农业开源大模型与 RAGFlow、Dify 等 RAG 框架已成熟可用；ESP32 单芯片即可完成数据采集、拍照与控制，Node.js + Vue 全栈平台生态成熟，硬件与软件成本都低，整条链路可以自研。"));
...
children.push(numItem("**硬件与嵌入式链路：**ESP32 生态成熟（arduino-esp32 官方核心，17k+ 星，持续更新）；ESP32-CAM 单板集成摄像头、Wi-Fi 与 microSD，外接传感器/继电器的整机案例丰富；现有 I2C、SPI、串口、PWM 知识可直接迁移。"));
children.push(numItem("**视觉检测链路：**[PlantDoc](https://github.com/pratikkayal/PlantDoc-Dataset) 数据集有官方 GitHub 仓库（427 星，CODS-COMAD 2020 论文配套）；PlantVillage 数据集在 Kaggle 公开（5 万+ 图、38 类）；YOLOv8 框架成熟（Ultralytics 60k 星），已有温室作物病害检测、ESP32-CAM 实时检测全链路先例。"));
children.push(numItem("**大模型/智能体链路：**司农（南京农业大学，国内首个农业开源大语言模型，8B/32B）与稷丰 [AgriAgent](https://github.com/zhiweihu1103/AgriAgent)（125 星，中文农业多模态）均已核验；RAGFlow/Dify 提供 HTTP API，Node.js 后端可直接调用，因此「智能体 → HTTP → MQTT → 设备」路径可行。"));
children.push(numItem("**平台链路：**[Express](https://github.com/expressjs/express)（69k 星）、[MQTT.js](https://github.com/mqttjs/MQTT.js)（9k 星）、[Vue 3](https://github.com/vuejs/core)（54k 星）、[ECharts](https://github.com/apache/echarts)（67k 星）均已核验；[EMQX](https://github.com/emqx/emqx)（16.5k 星）与 OneNET 支持 ESP32 直连，教程量大。"));
children.push(numItem("**已知约束（写进风险）：**ESP32-CAM 引脚少、供电敏感，需稳压供电与 I/O 规划；边缘算力有限，初期视觉推理走云端；司农 32B 本地部署显存要求高，初期用 8B 或 DeepSeek API 替代。"));

// ── 6 开源项目清单 ──
children.push(heading("6. 开源项目清单", 1));
children.push(body("核验方式：2026-08-07/08-10 通过 GitHub API 查询仓库全名、star 数、最近推送、是否归档；模型与数据集通过官方渠道与多家媒体报道交叉确认。"));

children.push(heading("6.1 硬件 / 整机（ESP32 侧）", 3));
children.push(bullet("[espressif/arduino-esp32](https://github.com/espressif/arduino-esp32)（17.2k 星，2026-08 更新，已核验）：乐鑫官方 Arduino 核心，ESP32-CAM 开发与示例的基准环境。"));
children.push(bullet("[SHN2004/Plant_Disease_Detection](https://github.com/SHN2004/Plant_Disease_Detection)（9 星，已核验）：ESP32-CAM + 深度学习实时植物病害检测全链路，与本项目视觉链路直接对口。"));
...
children.push(bullet("[langgenius/dify](https://github.com/langgenius/dify)（151k 星，已核验）：开源智能体/工作流平台，HTTP API 可被 Node.js 后端调用（可选编排层）。"));
children.push(bullet("[infiniflow/ragflow](https://github.com/infiniflow/ragflow)（87k 星，已核验）：开源 RAG 引擎，HTTP API 可被 Node.js 后端调用。"));
children.push(bullet("[ultralytics/ultralytics](https://github.com/ultralytics/ultralytics)（60k 星，已核验）：YOLOv8/YOLO11 框架（视觉服务为独立 Python 服务，由 Node.js 编排调用）。"));

children.push(heading("6.5 平台 / 前端 / 可视化（Node.js + Vue）", 3));
children.push(bullet("[expressjs/express](https://github.com/expressjs/express)（69.4k 星，已核验）：Node.js Web 框架，REST/WebSocket API 底座。"));
children.push(bullet("[mqttjs/MQTT.js](https://github.com/mqttjs/MQTT.js)（9.1k 星，已核验）：Node.js MQTT 客户端，云端接入层。"));
children.push(bullet("[vuejs/core](https://github.com/vuejs/core)（Vue 3，54.2k 星，已核验）：前端框架，大屏/控制页/问答界面。"));
children.push(bullet("[apache/echarts](https://github.com/apache/echarts)（67k 星，已核验）：可视化图表库，实时曲线与看板。"));
children.push(bullet("[emqx/emqx](https://github.com/emqx/emqx)（16.5k 星，已核验）：MQTT Broker。"));
children.push(bullet("[OneNET](https://open.iot.10086.cn)（中国移动物联网开放平台，已核验）：免费云平台，ESP32 MQTT 直连教程量大。"));
...
children.push(numItem("**低成本单芯片：**ESP32 单芯片完成采集、拍照、控制与联网，链路最短、硬件成本低，整套闭环可复现，适合竞赛展示与教学科研场景推广。"));
...
children.push(checkbox("采购/整理 ESP32-CAM 与传感器，跑通 M1：传感器采集 → MQTT → Node.js → Vue 控制页"));
children.push(checkbox("搭建 Node.js（Express + mqtt.js）+ Vue 3（Vite + ECharts）脚手架与本地 EMQX"));
children.push(checkbox("搭建 Python + YOLOv8 环境，用 PlantDoc 数据集跑通识别 demo"));

const doc = new Document({
  title: "SmartFarmAgent 智慧农业智能体系统——项目开题链路",
  subject: "项目开题链路（整体重写 v2.0）",
  description: "智慧农业智能体项目开题链路：ESP32 单芯片感知执行端 + Node.js/Vue 平台 + 云端大模型智能体。版本声明与备注见 VERSIONS.md。",
  creator: "LIM",
  styles: {
    default: {
      document: {

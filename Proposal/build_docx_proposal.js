...
children.push(bullet("行业需求真实：青海三江集团（省属国有大型农牧企业，396.86 万亩土地、282 万亩草场、23 万亩青稞油菜基地，年出栏牛羊超 7 万头）已系统规划数字底座、农机调度、粮仓物联网与 AI 智能体等建设内容，规模化智能生产是明确方向。"));
children.push(bullet("方案缺口：企业级智慧农业平台功能完整但重软件集成、成本高；市面常见产品以阈值规则控制为主，大模型应用停留在“知识问答”层，均未把智能决策闭环到设备执行。"));
children.push(bullet("技术窗口：2026 年智慧农业创新大赛六个赛道（智能农机作业控制、无人机巡田、设施采摘机器人、家禽巡检、鱼群检测）集中在“智能体 / IoT + 机器人”方向；农业开源大模型与 RAG 框架成熟可用，硬件成本低，整条链路可以自研。"));

// 2 行业对标与项目定位
children.push(heading("2. 行业对标与项目定位", 1));
children.push(body("本项目不做企业级全平台，而是以低成本硬件原型复现企业方案中最核心、最难落地的“感知—决策—执行”闭环。与青海三江集团智慧农业规划的模块对应关系如下："));
children.push(table(
  ["本项目模块", "企业方案对应模块（青海三江集团）", "本项目做法"],
  [
    ["自动浇水/施肥", "数字农田、水肥一体机（土壤墒情监测）", "STM32 依据土壤湿度+光照综合判断，继电器驱动水泵/蠕动泵"],
    ["环境自动调控", "物联网管理（气象、温湿度传感）", "DHT11/BH1750 采集，风扇/加热/补光灯联动"],
    ["病虫害检测+给药", "病虫害 AI 识别、虫情测报灯", "ESP32-CAM 定拍 + YOLO 识别，喷药泵执行"],
    ["智能体决策+农技问答", "AI 智能体（语音填报、RAG 农技问答、测土配方）", "LLM+RAG 输出设备指令与管护方案，Dify 工作流"],
    ["数据中台/展示", "集团决策驾驶舱、数字良田、报表分析", "MQTT 上行 + 时序存储 + 小程序/H5"],
    ["断网兜底", "高原弱网适配（离线缓存、断点续传）", "本地规则优先，W25Q64 日志断网补传"],
  ],
  [2400, 3600, 3072],
  [AlignmentType.CENTER, AlignmentType.LEFT, AlignmentType.LEFT]
));
children.push(spacer());
children.push(body("**定位：**以 STM32 感知执行端 + 云端大模型智能体，做企业级智慧农业“平台能力的最小可验证原型”，聚焦智能决策到设备动作的完整闭环。"));
...
children.push(table(
  ...
  [
    ...
    ["农技知识问答", "种植/病虫害/用药知识库 → RAG 检索增强 → 结构化方案+来源标注", "中", "司农/DeepSeek + Dify/RAGFlow 已核验，与三江方案中 AI 农技问答同构"],
...
children.push(numItem("智能体链路：司农（国内首个农业开源大语言模型，8B/32B）与 AgriAgent（125 星）均已开源；Dify 官方确认 HTTP 节点可调外部 API，智能体到设备路径可行；青海三江集团规划中同类 AI 智能体（语音填报、RAG 农技问答、病虫害识别）已进入企业建设，技术路线成熟。"));
...
children.push(numItem("低成本对标：以低成本硬件原型复现企业级方案（青海三江集团）的核心闭环，教学与科研场景可复制。"));
...
const doc = new Document({
  ...
  subject: "项目方案（导师提交版，v1.6）",
  description: "导师提交版 v1.6：背景、行业对标与定位、架构、功能、数据链路、可行性、里程碑、创新点、风险与主要开源参考。",
  creator: "LIM",
  styles: {
    default: {
      document: {
